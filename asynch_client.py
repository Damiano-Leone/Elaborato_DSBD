...
async def node_memory_MemAvailable_bytes(stub: retrieval_pb2_grpc.RetrievalServiceStub) -> None:
    print( "Metrica 2: node_memory_memAvail_bytes\n")
                
    request = retrieval_pb2.RetrievalRequest(query='select * from dickey_fuller where nome="memory_memAvail_bytes"')
    print("-------------- GetDickeyFuller --------------\n")
    await get_dickey_fuller(stub, request)
                
    request = retrieval_pb2.RetrievalRequest(query='select * from decomposition where nome="memory_memAvail_bytes"')
    print("-------------- ListDecomposition --------------\n")
    await list_decomposition(stub, request)
                
    request = retrieval_pb2.RetrievalRequest(query='select * from hodrick_prescott where nome="memory_memAvail_bytes"')
    print("-------------- ListHodrickPrescott --------------\n")
    await list_hodrick_prescott(stub, request)
                
    request = retrieval_pb2.RetrievalRequest(query='select * from autocorrelation where nome="memory_memAvail_bytes"')
    print("-------------- ListAutocorrelation --------------\n")
    await list_autocorrelation(stub, request)
                
    request = retrieval_pb2.RetrievalRequest(query='select * from aggregates where nome="memory_memAvail_bytes"')
    print("-------------- ListAggregates --------------\n")
    await list_aggregates(stub, request)
    
    print( "Fine stampa Metrica 2: node_memory_memAvail_bytes\n")
